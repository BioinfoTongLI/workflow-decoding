...
def detect_and_extract_spots(
    imgs_coding,
    anchors,
    C,
    R,
    imgs_also_without_tophat=None,
    compute_also_without_tophat=False,
    norm_anchors=False,
    use_blob_detector=False,
    trackpy_diam_detect=5,
    ...
    if (
        len(anchors.shape) > 2
    ):  # if there are multiple frames, detect spots in each and link them
        # apply trackpy to each round
        if norm_anchors:
            anchors = (anchors - np.mean(anchors, axis=(1, 2), keepdims=True)) / np.std(
                anchors, axis=(1, 2), keepdims=True
            )
        tracks = trackpy.link_df(
            trackpy.batch(
                anchors, diameter=trackpy_diam_detect, percentile=trackpy_prc
            ),
        ...
        for ind_cy in range(1, R):
            spots_id_all_anchors = np.intersect1d(
                spots_id_all_anchors,
        ...
        if use_blob_detector:
            locs = blob_log(
                anchors,
                min_sigma=trackpy_diam_detect / 2,
                max_sigma=3 / 2 * trackpy_diam_detect,
                num_sigma=11,
                threshold=0.25,
            )
            locs_y = locs[:, 0]
    ...
    centers_c01 = np.transpose(
        np.stack((centers_x[:, 0], centers_y[:, 0]))
    )  # equivalent to the centers in matlab if + 1
        ...
        for dy in np.array([-1, 0, 1]):
            ...
            for ind_cy in range(R):
                ...
                y_coord = np.maximum(
                    0,
                    np.minimum(
                        imgs_coding.shape[0] - 1,
                        np.around(centers_y[:, ind_cy]).astype("int32") + dy,
                    ),
                )
                for ind_ch in range(C):
                    spot_intensities_d[d, :, ind_ch, ind_cy] = imgs_coding[
            ...
            for dy in np.array([-1, 0, 1]):
                ...
                for ind_cy in range(R):
                    x_coord = np.maximum(
                        0,
                        np.minimum(
                            imgs_also_without_tophat.shape[1] - 1,
                            np.around(centers_x[:, ind_cy]).astype("int32") + dx,
                        ),
                    )
                    y_coord = np.maximum(
                        0,
                        np.minimum(
                            imgs_also_without_tophat.shape[0] - 1,
                            np.around(centers_y[:, ind_cy]).astype("int32") + dy,
                        ),
                    )
                    for ind_ch in range(C):
                        spot_intensities_d[
                            d, :, ind_ch, ind_cy
                        ] = imgs_also_without_tophat[y_coord, x_coord, ind_ch, ind_cy]
...
def load_tiles_to_extract_spots(
    tifs_path,
    channels_info,
    C,
    R,
    ...
    anchors_cy_ind_for_spot_detect=None,
    norm_anchors=False,
    use_blob_detector=False,
    compute_also_without_tophat=False,
    ...
    for y_ind in range(tiles_to_load["y_start"], tiles_to_load["y_end"] + 1):
        for x_ind in range(tiles_to_load["x_start"], tiles_to_load["x_end"] + 1):
            ...
            if np.isin(tile_name, tile_names["selected_tile_names"]):
                ...
                imgs = np.zeros(
                    (tile_size_y, tile_size_x, len(channels_info["channel_names"]), R)
                )
                for ind_cy in range(R):
                    for ind_ch in range(len(channels_info["channel_names"])):
                        if (
                            channels_info["channel_names"][ind_ch] != "DAPI"
                        ):  # no need for dapi
                            img_name = tifs_path + tile_name_pattern.format(
                                prefix=tiles_info["filename_prefix"],
                ...
                imgs_coding = imgs[
                    :, :, np.where(np.array(channels_info["coding_chs"]) == True)[0], :
                ]
                # apply top-hat filtering to each coding channel
                imgs_coding_tophat = np.zeros_like(imgs_coding)
                for ind_cy in range(R):
                    for ind_ch in range(C):
                        imgs_coding_tophat[:, :, ind_ch, ind_cy] = white_tophat(
                            imgs_coding[:, :, ind_ch, ind_cy],
                ...
                if anchor_available:
                        ...
                        np.swapaxes(
                            np.squeeze(
                                imgs[
                                    ...
                                    :,
                                    np.where(
                                        np.array(channels_info["channel_base"])
                                        == "anchor"
                                    )[0][0],
                                    :,
                                ]
                    ...
                    imgs_coding_tophat_norm = (
                        imgs_coding_tophat
                        - np.min(imgs_coding_tophat, axis=(0, 1), keepdims=True)
                    ) / (
                        np.percentile(
                            imgs_coding_tophat, 99.99, axis=(0, 1), keepdims=True
                        )
                        - np.min(imgs_coding_tophat, axis=(0, 1), keepdims=True)
                    )
                    anchors = np.swapaxes(
                        np.swapaxes(imgs_coding_tophat_norm.max(axis=2), 0, 2), 1, 2
                    )

                anchors = anchors[
                    anchors_cy_ind_for_spot_detect, :, :
                ]  # select only those cycles given in anchors_cy_ind_for_spot_detect

                # detect and extract spots from the loaded tile
                spots_i, centers_i, spots_notophat_i = detect_and_extract_spots(
                    imgs_coding_tophat,
                    anchors,
                    C,
                    R,
                ...
                if N_i > 0:
                    ...
                    if compute_also_without_tophat:
                        spots_notophat = np.concatenate(
                            (spots_notophat, spots_notophat_i)
                        )
                    # saving spots locations from the 1st cycle in a data frame (needed for ploting)
                    X = (x_ind - tiles_to_load["x_start"]) * tiles_info[
                        "tile_size"
                    ] + centers_i[:, 0]
                    Y = (y_ind - tiles_to_load["y_start"]) * tiles_info[
                        "tile_size"
                    ] + centers_i[:, 1]
