...
def main(args):
    ...
    tiles_to_load = {
        ...
        "x_end": args.x_range[1],
    }  # select which tiles to load, including indices at the end
    tiles_info = {
    ...
    spots, spots_loc, _ = load_tiles_to_extract_spots(
        ...
        tiles_to_load,
        spots_params,
        args.tile_name_pattern,
        anchor_available=bool(args.anchor_available),
        coding_ch_starts_from=args.coding_ch_starts_from,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tile_name_pattern", type=str, required=True)
    parser.add_argument("-anchor_available", type=int, required=True)
    ...
    parser.add_argument("-trackpy_percentile", type=int, required=True)
    parser.add_argument("-channel_info", type=str, required=True)
    parser.add_argument("-coding_ch_starts_from", type=int, required=True)
    parser.add_argument("-y_range", type=int, nargs=2, required=True)
    parser.add_argument("-x_range", type=int, nargs=2, required=True)
